...
DietBot 已整合 LINE 官方帳號，您可以直接加入好友體驗與使用：

- **LINE 官方帳號 ID**：`@076rfleq`
- **加入好友方式**：開啟 LINE App ➔ 主頁 ➔ 點選右上角「加入好友」 ➔ 搜尋 ID 輸入 `@076rfleq` 並新增好友。

### 📖 使用流程與指令指南

1. **加入好友與初始化**：
   - 搜尋並新增 `@076rfleq` 好友後，加入好友時系統將自動於背景完成使用者帳號初始化，不彈出預設迎賓訊息，提供簡潔順暢的對話體驗。

2. **設定個人體重與減重/增重目標**：
   - 輸入自然語言設定，例如：
     > 「我現在 75kg，想減到 68kg」
   - AI 自動計算並儲存您的每日 BMR、TDEE、目標熱量與三大營養素（蛋白質、碳水化合物、脂肪）預算。

3. **動態階段式餐點引導與紀錄**：
   - 🌅 **早餐建議與紀錄**：傳送「早安」獲取早餐建議選單；傳送照片或文字（如：「早餐吃了鮪魚蛋吐司和無糖拿鐵」）進行紀錄。
   - ☀️ **午餐建議與紀錄**：紀錄早餐後自動獲得午餐建議卡片；吃完後傳送照片或文字進行紀錄。
   - 🌙 **晚餐建議與紀錄**：紀錄午餐後自動獲得晚餐建議卡片；吃完後傳送照片或文字進行紀錄。
   - 📊 **今日總結**：完成晚餐紀錄後，自動生成【一日總結與明日建議】報告。
   - 🧁 **點心/宵夜**：隨時傳送點心照片或文字描述，自動採計入當日熱量而不打亂主流程。

4. **常用快捷指令**：
   - **「運動」** 或 **「今日運動」**：獲得個人化計畫性運動推薦。
   - **「今日紀錄」** 或 **「總覽」**：即時查看今日熱量與三大營養素進度條 `[██████░░░]` 及剩餘預算。
   - **「說明」** 或 **「幫助」**：顯示機器人詳細功能選單與使用指導。
